...
const Register = () => {
  ...
  const [toast, setToast] = useState({
    message: '',
    type: 'info',
    open: false, // Ensure 'open' is part of the toast state
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.income && isNaN(formData.income)) validationErrors.income = 'Income must be a valid number.';
    if (formData.credit_score && (isNaN(formData.credit_score) || formData.credit_score < 300 || formData.credit_score > 850)) validationErrors.credit_score = 'Credit score must be between 300 and 850.';
    if (formData.employment_status && formData.employment_status.length < 2) validationErrors.employment_status = 'Employment status must be at least 2 characters long.';
    if (formData.email && !/\S+@\S+\.\S+/.test(formData.email)) validationErrors.email = 'Email is invalid';
    ...
    if (formData.date_of_birth) {
      ...
      if (age < 18) validationErrors.date_of_birth = 'You must be at least 18 years old.';
    }

    const passwordRegex = /^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=[\]{};':"\\|,.<>\/?]).{6,}$/;
    if (formData.password && !passwordRegex.test(formData.password)) validationErrors.password = 'Password must contain at least 6 characters, one uppercase letter, one number, and one special character.';
    ...
    try {
      ...
      if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        data = { message: text || 'Registration failed due to an unexpected server response.' };
      }

      if (response.ok) {
        setToast({ message: 'Registration successful!', type: 'success', open: true });
        setTimeout(() => {
          navigate("/login");
        }, 1000);
      } else {
        // Display server-side error message
        setToast({ message: data.message || 'Registration failed. Please try again.', type: 'error', open: true });
      }
    } catch (error) {
  ...
  return (
    <>
      <Toast
        open={toast.open} // Control the visibility of the toast
        type={toast.type}
